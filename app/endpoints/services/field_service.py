...
class FieldService:
    ...
    async def get_nearby_fields(self, query_point: str, radius: float, crop: t.Optional[str]) -> t.List:
        stmt = (
            select(
                self.MODEL.id,
                self.MODEL.crop,
    ...
    async def get_fields_in_parallelogram(self, parallelogram: Polygon, crop: t.Optional[str]) -> t.List:
        stmt = select(
            self.MODEL.id,
            self.MODEL.crop,
            self.MODEL.productivity,
    ...
    async def get_fields_intersecting_geometry(self, figure: Polygon, crop: t.Optional[str]) -> t.List:
        stmt = select(
            self.MODEL.id,
            self.MODEL.crop,
            self.MODEL.productivity,
    ...
    async def get_agricultural_metrics(self, region: str) -> t.List:
        stmt = (
            select(
                ...
                func.coalesce(func.sum(Field.productivity * Field.area_ha) / func.sum(Field.area_ha), 0).label(
                    "average_yield")
            )
